...
//logpid
static volatile int m_log_pid_speed_cnt = 0;
static volatile bool m_log_pid_speed = true;
 ...
 */
static void control_current(volatile motor_all_state_t *motor, float dt) {
	volatile motor_state_t *state_m = &motor->m_motor_state;//电机实时状态结构体
	volatile mc_configuration *conf_now = motor->m_conf;//参数结构体

	float c,s;
	utils_fast_sincos_better(state_m->phase, &s, &c);//转子电角度的正余弦计算
...
//	utils_truncate_number((float*)&state_m->vq_int, -max_v_mag, max_v_mag);
	...
	// 		commands_printf("%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n",
